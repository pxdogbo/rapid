...
| `/rapid plan [<slug>] <task>` | **Request a rapid-plan: a plan delivered through collab. Plan authoring runs on the STRONGEST model — never down to a cheaper rider.** No slug (the token after `plan` isn't an existing session slug) → the whole text is the task and YOU author the plan, in your own chat; in a collab that's the lead, which then hands lanes to the riders. With `<slug>` → `collab_send` the brief and the peer authors it — sanctioned directions are **up or across** (equal/stronger model, or the peer owning the context); if `<slug>` names a rider in your own collab, author it yourself instead and say so in one line (an explicit user override wins). The peer sends it back through the room (`[plan] …`); you review, then surface. A **rapid-plan is NOT the built-in Plan subagent or plan mode** — never spawn one for it. When the user says "rapid-plan" (or "have <slug> plan it"), this flow is what they mean. See `references/collab.md` → "rapid-plan". |
